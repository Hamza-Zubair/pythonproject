...
#This function returns the number of weeks of the in between input of 2 dates
...
def makeweeklist(spoint, wlist): 
    if (spoint == wlist): 
        ...
        while(spoint < wlist+1 ): 
            ...
            spoint += 1
        return res 
#This Function use random function to assign number of weeks evenly to the teams/tenants
...
Teams=[a,b,c,d] # Saving Names of Tenants
email = [a1,b1,c1,d1] #Saving Email Addresses of Tenants
startdate = datecon(userinput) #Start Date of Contract
enddate = datecon(userinput2) #End Date of Contract
...
days=[] 
dates=[]
daycreator(days,FinalDic) #Converting week numbers to days
date_creator(datetime_obj1,days) #converting those days to dates
lastdic={} #final output which will be used for email function
emails_dict={} #this will create dictionary out of 2(tenant names and the tenant email addresses)

for z in range(4):
    emails_dict[Teams[z]]=email[z]
...
def send_email(email_list):
    ...
    smtpObj.ehlo()
    smtpObj.starttls()
    smtpObj.login('hamza.ut100@gmail.com','muhammadhamza')
    for each_mail in email_list:
        body=''' \nSubject: Cleaning notification\n
            Dear Tenant,\n
            Today is your turn for cleaning '''
        print('Email is sending...')
        sendmailStatus=smtpObj.sendmail('sender email',each_mail,body.encode("utf-8"))
        
        if sendmailStatus !={}:
            print('E-mail failed %s: %s' %(each_mail,sendmailStatus))
    smtpObj.quit()
...
def create_notification():
    ...
    for key,value in lastdic.items():
        if today in value:
            tenant_name=key
            tenant_email=emails_dict[key]
            target_emails.append(tenant_email)
